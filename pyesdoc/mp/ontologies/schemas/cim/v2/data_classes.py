...
def dataset():
    ...
    return {
        ...
        'properties': [
            ...
            ('progress', 'iso.md_progress_code','0.1', 'State of the dataset'),
            ('keywords', 'str', '0.N', 'Set of additional keywords to aid discovery/classification'),
            ('further attributes', 'shared.extra_attribute','0.N',
                'Additional attributes as necessary.'),
            ('lineage','linked_to(iso.lineage)', '0.1', 'Provenance of the dataset')
        ]
    }
...
def dataset_type():
    ...
    return {
        ...
        'is_open': False,
        'members': [
            ('reanalysis', 'Hindcast which includes observations via data assimilation'),
            ('analysis', 'Product of manipulation of multiple input datasets'),
            ('forecast', 'Representation of a future real time predicted from a specific initial condition '),
            ('hindcast', 'Representation of a past real time predicted from a specific initial condition'),
            ('projection', 'Representation a possible future given initial conditions and assumptions'),
            ('representation', 'Simulation of a particular object or process'),
            ('observation', 'Constructed from observations or measurements alone'),
            ('dump', 'Raw computer output intended for re-use within or by originating software'),
            ('modified', 'Modified representation of another dataset (e.g. regridded)'),
            ('unphysical', 'Not intended for comparison with the real world (e.g. as part of a sensitivity study)'),
            ('downscaled', 'Dataset was downscaled by embedded simulation within driving data at larger scale'),
        ]
    }
